import { Body, Controller, UseGuards, Post, Get, Req, Headers, HttpException, HttpStatus, Response, Patch, Put, ValidationError, ValidationPipe } from '@nestjs/common';
...
export class QuizController {
    ...
    @Get()
    selectQuizzes(@Req() request: RequestWithUser) {
        try {
            const uidUser = request.user.uid;
            const listeQuiz = this.quizzService.getAllQuiz(uidUser);

            return listeQuiz;

        } catch (error) {
            console.log("Error Get All Quiz => ", error);
            throw new HttpException('Unauthorized', HttpStatus.UNAUTHORIZED);
        }
    }
